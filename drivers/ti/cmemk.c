...
#endif
{
    ...
    switch (cmd & CMEM_IOCCMDMASK) {
		...
		//__E("npools %d, bi %i\n", npools[bi], bi);
            ...
            if (pool == -1) {
                __E("CMEM_IOCGETPOOL Failed to find a pool which fits %d\n", reqsize);
...
/* cut-and-paste above as part of adding support for more than 2 blocks */
        ...
        for (i=0; i<npools[bi]; i++) {
        	...
        	//printk(KERN_INFO "alloc pool: bi %i %i %i %i\n",bi, i, pool_num_buffers, pool_size );
        ...
        printk(KERN_INFO "cmem initialized %d pools between %#lx and %#lx\n",

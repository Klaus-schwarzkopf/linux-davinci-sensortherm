				    ...
				    unsigned char ram_clut[256][3])
{
	...
	for (i = 0; i < 256; i++) {
		osd_device->ops.set_clut_ycbcr(osd_device, i,
					       ram_clut[i][0],
					       ram_clut[i][1],
					       ram_clut[i][2]);
	}
				  ...
				  bitmap_conf_params)
{
	...
	if (bitmap_conf_params->clut_select == 0)
		osd_device->ops.set_rom_clut(osd_device,
				 ROM_CLUT0);
	...
	osd_device->ops.set_rec601_attenuation(osd_device,
			 osdwin, bitmap_conf_params->attenuation_enable);
...
static int vpbe_fb_set_par(struct fb_info *info)
{
	...
	osd_device->ops.set_layer_config(osd_device,
			 win->layer, &lconfig);
	osd_device->ops.start_layer(osd_device, win->layer,
			 start, fb_cbcr_ofst);
			       ...
			       struct fb_info *info)
{
	...
	osd_device->ops.set_clut_rgb(osd_device, regno,
			 r, g, b);
...
static int vpbe_fb_probe(struct platform_device *pdev)
{
	...
	err =
	    vpbe_fb_init_window(&pdev->dev, &dm->win[WIN_OSD0],
	...
	dm->win[WIN_VID0].sdram_address = 0;
	if (!vpbe_fb_get_default_win_config
	    (&pdev->dev, &dm->win[WIN_VID0], &lconfig, &fb_size, options)) {
		err =
		    vpbe_fb_init_window(&pdev->dev, &dm->win[WIN_VID0],
	...
	vpbe_fb_get_default_win_config(&pdev->dev, &dm->win[WIN_OSD1],
				       &lconfig, &fb_size, options);
	err =
	    vpbe_fb_init_window(&pdev->dev, &dm->win[WIN_OSD1],
	...
	dm->win[WIN_VID1].sdram_address = 0;
	if (!vpbe_fb_get_default_win_config
	    (&pdev->dev, &dm->win[WIN_VID1], &lconfig, &fb_size, options)) {
		err =
		    vpbe_fb_init_window(&pdev->dev, &dm->win[WIN_VID1],
